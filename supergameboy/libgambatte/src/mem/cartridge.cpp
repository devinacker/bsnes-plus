...
LoadRes Cartridge::loadROM(bool const forceDmg,
                           bool const multicartCompat)
{
	...
	rombanks = pow2ceil(supergameboy.romsize / rombank_size());
	...
	memcpy(reinterpret_cast<char*>(memptrs_.romdata()), supergameboy.romdata, 
	       supergameboy.romsize / rombank_size() * rombank_size());
	std::memset(memptrs_.romdata() + supergameboy.romsize / rombank_size() * rombank_size(),
	            0xFF,
	            (rombanks - supergameboy.romsize / rombank_size()) * rombank_size());
...
void Cartridge::loadSavedata() {
	...
	if (hasRtc(memptrs_.romdata()[0x147])) {
		if (supergameboy.rtcdata && supergameboy.rtcsize >= 4) {
			unsigned long basetime =   (supergameboy.rtcdata[0]);
